...
AS $$
  ...
  paged AS (
    ...
    FROM user_base
    CROSS JOIN params
    ORDER BY user_base.created_at DESC, user_base.id DESC
...
REVOKE ALL ON FUNCTION public.admin_list_users_page(text, text, integer, integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.admin_list_users_page(text, text, integer, integer) TO service_role;
