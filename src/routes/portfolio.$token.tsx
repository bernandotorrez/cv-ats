...
export const Route = createFileRoute("/portfolio/$token")({
  ...
  head: ({ loaderData }) => {
    ...
    return buildSeo({
      ...
      path: `/portfolio/${loaderData.token}`,
      noindex: true,
    });
  },
  component: SharePage,
...
function DescriptionBlock({ text }: { text: unknown }) {
  ...
  return (
    <ListTag
      className={
        isNumbered
          ? "mt-4 list-decimal space-y-2 pl-5 text-sm leading-7 text-muted-foreground"
          : "mt-4 list-disc space-y-2 pl-5 text-sm leading-7 text-muted-foreground"
      }
    >
